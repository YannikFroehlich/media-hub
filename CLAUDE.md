...
- Gamepads (`core/gamepad.ts`, standard mapping) are replayed as synthetic `keydown` events on the focused element — D-pad/left stick = arrows, A = Enter (clicks if nothing handled it), B = Escape — so they reuse all keyboard handling. App ignores arrows while a modal is open (keyboard users keep native select/text/range behavior), so for the pad `moveFocusInDialog()` walks the topmost modal's controls in tab order instead (pulling focus back in if it slipped outside). Script-driven focus never matches `:focus-visible`, so pad input sets `data-input="gamepad"` on `<html>` (cleared by any trusted keydown/pointerdown) and `_base.scss`/`_forms.scss` show the focus ring on plain `:focus` in that mode.
